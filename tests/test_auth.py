...
async def test_valid_key_passes(async_session, monkeypatch):
    ...
    request = _request_with_key(raw_key)
...
async def test_revoked_key_returns_401(async_session, monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        await get_api_key(_request_with_key(raw_key))
    assert exc.value.status_code == 401
...
async def test_missing_header_returns_401():
    with pytest.raises(HTTPException) as exc:
        await get_api_key(_request_with_key(None))
    assert exc.value.status_code == 401


@pytest.mark.asyncio
async def test_last_used_at_is_updated_asynchronously(async_session, monkeypatch):
    _patch_session_factory(monkeypatch, async_session)
    raw_key = "touch-key"
    row = ApiKey(
        key_hash=blake3(raw_key.encode("utf-8")).hexdigest(),
